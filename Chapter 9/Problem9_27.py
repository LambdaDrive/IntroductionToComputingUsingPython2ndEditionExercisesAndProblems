...
def getresult(event=0):
    global entryresult, result, expression, tries, entrycalculate, problemlist
    if int(entryresult.get()) == result:
        showinfo(message = 'Number of tries = {} \n You got it!'.format(tries))
        expression = generateproblem()
        while True:
            if expression in problemlist:
                expression = generateproblem()
            if expression not in problemlist:
                break
        if len(problemlist)>= 10:
            problemlist.pop(0)
        problemlist.append(expression)
